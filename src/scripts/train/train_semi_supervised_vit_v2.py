...
from src.models.Unet_ViT import U_net_ViT
...
def main():
    ...
    # 1) Build the NEW UNet+ViT student model
    student = U_net_ViT(
    ...
    # 2) Wrap it in the Mean Teacher container
    model = MeanTeacherUNetV2(
        student_model=student,
        ema_alpha=args.ema_alpha
    ).to(device)

    # 3) Initialize weights (same as before)
