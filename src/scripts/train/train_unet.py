...
for epoch in range(args.epochs):
    ...
    for img, mask in train_loader:
        ...
        if p.dim() == 4 and mask_f.dim() == 4:
            # CROP PRED + MASK TO MATCH NEW UNET OUTPUT (560x560)
...
# Save model
mp=os.path.join(args.save_dir,"unet_trained.pth"); os.makedirs(args.save_dir,exist_ok=True)
...
pm_vis_np = pm_vis.detach().cpu().numpy()
img_vis = img_e.squeeze(0).cpu().numpy()
mask_vis = mask_e.squeeze(0).cpu().numpy()

h = min(pm_vis_np.shape[0], img_vis.shape[0], mask_vis.shape[0])
w = min(pm_vis_np.shape[1], img_vis.shape[1], mask_vis.shape[1])
